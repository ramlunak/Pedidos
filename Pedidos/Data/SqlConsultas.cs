...
namespace Pedidos.Data
{
    public static class SqlConsultas
    {
        ...
        public static string GetSqlProductosAll(int idCuenta)
        {
            ...
            query.Append($" ,JsonAdicionales = (SELECT * FROM (SELECT value  FROM STRING_SPLIT(CAST ((select top(1) idsAdicionales from [dbo].[P_CategoriaAdicional] where idCategoria = P.idCategoria and idCuenta = {idCuenta}) AS varchar(MAX)), ',') WHERE RTRIM(value) <> '') AS V JOIN[dbo].[P_Adicionais] AS A on V.value = A.id WHERE A.activo = 1 FOR JSON PATH) ");
            query.Append($" ,JsonIngredientes = (SELECT * FROM (SELECT value  FROM STRING_SPLIT(CAST ((select top(1) idsIngrediente from [dbo].[P_IngredientesProducto] where idProducto = P.id and idCuenta = {idCuenta}) AS varchar(MAX)), ',') WHERE RTRIM(value) <> '') AS V JOIN [dbo].[P_Ingredientes] AS A on V.value = A.id WHERE A.activo = 1 FOR JSON PATH)  ");
            query.Append($" ,JsonSabores = (SELECT * FROM (SELECT value  FROM STRING_SPLIT(CAST ((select top(1) idsSabor from [dbo].[P_SaboresProducto] where idProducto = P.id and idCuenta = {idCuenta}) AS varchar(MAX)), ',') WHERE RTRIM(value) <> '') AS V JOIN [dbo].[P_Sabores] AS A on V.value = A.id WHERE A.activo = 1 FOR JSON PATH)  ");
            query.Append($" FROM[dbo].[P_Productos] AS P ");
            query.Append($" WHERE P.idCuenta = {idCuenta} ");
            var str = query.ToString();
            return str;
        ...
        public static string GetSqlProductosDetalle(int idCuenta, int? idProducto)
        {
            ...
            query.Append($" SELECT P.id as id");
            query.Append($" ,(SELECT * FROM (SELECT value  FROM STRING_SPLIT(CAST ((select top(1) idsAdicionales from [dbo].[P_CategoriaAdicional] where idCategoria = P.idCategoria and idCuenta = {idCuenta}) AS varchar(MAX)), ',') WHERE RTRIM(value) <> '') AS V JOIN[dbo].[P_Adicionais] AS A on V.value = A.id WHERE A.activo = 1 FOR JSON PATH) as JsonAdicionales ");
            query.Append($" ,(SELECT * FROM (SELECT value  FROM STRING_SPLIT(CAST ((select top(1) idsIngrediente from [dbo].[P_IngredientesProducto] where idProducto = P.id and idCuenta = {idCuenta}) AS varchar(MAX)), ',') WHERE RTRIM(value) <> '') AS V JOIN [dbo].[P_Ingredientes] AS A on V.value = A.id WHERE A.activo = 1 FOR JSON PATH) as JsonIngredientes  ");
            query.Append($" ,(SELECT * FROM (SELECT value  FROM STRING_SPLIT(CAST ((select top(1) idsSabor from [dbo].[P_SaboresProducto] where idProducto = P.id and idCuenta = {idCuenta}) AS varchar(MAX)), ',') WHERE RTRIM(value) <> '') AS V JOIN [dbo].[P_Sabores] AS A on V.value = A.id WHERE A.activo = 1  FOR JSON PATH) as JsonSabores  ");
