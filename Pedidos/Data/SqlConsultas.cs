...
namespace Pedidos.Data
{
    public static class SqlConsultas
    {
        ...
        public static string GetSqlProductosDetalle(int idCuenta)
        {
            StringBuilder query = new StringBuilder();

            query.Append($" SELECT P.id as id");
            query.Append($" ,(SELECT * FROM (SELECT value  FROM STRING_SPLIT(CAST ((select top(1) idsAdicionales from [dbo].[P_CategoriaAdicional] where idCategoria = P.idCategoria and idCuenta = {idCuenta}) AS varchar), ',') WHERE RTRIM(value) <> '') AS V JOIN[dbo].[P_Adicionais] AS A on V.value = A.id FOR JSON AUTO) as JsonAdicionales ");
            query.Append($" ,'' as JsonIngredientes ");
