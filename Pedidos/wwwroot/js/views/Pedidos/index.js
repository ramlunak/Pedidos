...
function TABLE_PedidosPendientes() {
    ...
    $.each(_PedidosPendientes, function (index, pedido) {
        ...
        $.each(pedido.productos, function (index, producto) {
            ...
            $('#minutes_' + pedido.id + '_' + index + '_' + producto.id + '').html(null);
            $('#seconds_' + pedido.id + '_' + index + '_' + producto.id + '').html(null);

            setInterval(function () {

                let minutes_ = (sec / 60).toFixed(0);
                let seconds_ = sec % 60;

                $('#minutes_' + pedido.id + '_' + index + '_' + producto.id + '').html(minutes_);
                $('#seconds_' + pedido.id + '_' + index + '_' + producto.id + '').html(seconds_);
                sec++;
            }, 1000);

            $('#div_' + pedido.id + '_' + index + '_' + producto.id + '').remove();

            var tiempo = '<div id="div_' + pedido.id + '_' + index + '_' + producto.id + '" class="border border-warning rounded p-1 d-flex align-items-center mr-1 ml-1" style="font-size: 10px;color: darkcyan;font-weight: 700;"> <i class="far fa-clock mr-1"></i> <span id="minutes_' + pedido.id + '_' + index + '_' + producto.id + '"></span>: <span id="seconds_' + pedido.id + '_' + index + '_' + producto.id + '"></span> <a data-toggle="tooltip" data-placement="top" title="Marcar Preparado"><i class="fas fa-check mr-2 ml-2 cursor-pointer"></i></a> </div>';
            //FIN
